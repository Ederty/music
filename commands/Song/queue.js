      ...
      if(player && botchannel && channel.id !== botchannel.id){
        ...
        try{
            ...
            if (!tracks.length)
              return message.channel.send(new MessageEmbed()
                .setTitle(`Queue for ${message.guild.name}`)
                .setFooter(`Page 1/1 | Loop: ${player.trackRepeat ? `✅` : `❌`} | Queue Loop: ${player.queueRepeat ? `✅` : `❌`}`, message.member.user.avatarURL({
                    dynamic: true,
                }))
                .setColor("ffffff")
            ...
            if (tracks.length < 10)
              return message.channel.send(new MessageEmbed()
                ...
                .addField(`__Now Playing:__`, `[${player.queue.current.title.substr(0, 60)}](${player.queue.current.uri}) | \`${player.queue.current.isStream ? `LIVE STREAM` : format(player.queue.current.duration).split(` | `)[0]} Requested by: ${player.queue.current.requester.tag}\`\n`)
                .setColor("ffffff")
                .addField(`__Up Next:__`, tracks.map((track, i) => `\`${++i}.\` [${track.title.substr(0, 60)}](${player.queue.current.uri}) | \`${track.isStream ? `LIVE STREAM` : format(track.duration).split(` | `)[0]} Requested by: ${track.requester.tag}\`\n`).join(`\n`)))
            //get an array of quelist where 15 tracks is one index in the array
            let quelist = [];
            for (let i = 0; i < tracks.length; i += 10) {
              let songs = tracks.slice(i, i + 8);
              quelist.push(songs.map((track, index) => `\`${i + ++index}.\` [${track.title.substr(0, 60)}](${player.queue.current.uri}) | \`${track.isStream ? `LIVE STREAM` : format(track.duration).split(` | `)[0]} Requested by: ${track.requester.tag}\`\n`).join(`\n`))
            }
            let limit = quelist.length <= 6 ? quelist.length : 6
            let embeds = []
            for (let i = 0; i < limit; i++) {
              ...
              await embeds.push(new MessageEmbed()
                .setTitle(`Queue for ${message.guild.name}`)
                .setFooter(`Page ${pageNumber} | Loop: ${player.trackRepeat ? `✅` : `❌`} | Queue Loop: ${player.queueRepeat ? `✅` : `❌`}`, message.member.user.avatarURL({
                    dynamic: true,
                }))
                .setColor("ffffff")
                .addField(`__Now Playing:__`, `[${player.queue.current.title.substr(0, 60)}](${player.queue.current.uri}) | \`${player.queue.current.isStream ? `LIVE STREAM` : format(player.queue.current.duration).split(` | `)[0]} Requested by: ${player.queue.current.requester.tag}\`\n`)
                .addField(`__Up Next:__` , desc)
                .addField(`\u200b`, `**${tracks.length} songs in queue | ${format(tracks.duration).split(" | ")[0]} total length**` ));
            }
